...
public class EventoDao {
	...
	public String editarEvento(Evento e) {
		...
		try {
			...
			ex.printStackTrace();
			msg = "Fallo al modificar el Evento";
		}
		return msg;
	}
	...
	public void borrarEvento(Evento e) throws Exception {
		...
		sql = "DELETE FROM Participacion WHERE id_evento = ? ;";
	...
	public int verRepetidos(String nom, int id_oli, String id_dep) throws Exception {
		String sql;
		int respuesta = -1;
		try {
			conexion = new ConexionBD();
			...
			sql = "SELECT count(*) as respuesta FROM Evento WHERE nombre = ? AND id_olimpiada = ? AND id_deporte = ?;";
